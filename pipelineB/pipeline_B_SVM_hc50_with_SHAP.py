...
imp.to_csv(os.path.join(BASE_DIR, "shap_pipeline_B_hc50_mean_abs.csv"))
...
for i, c in enumerate(classes):
    ...
    plt.title(f"SHAP mean |value| - {c} (HC50, Pipeline B)")
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f"shap_bar_pipeline_B_HC50_{safe(c)}.png"), dpi=150)
    plt.close()
