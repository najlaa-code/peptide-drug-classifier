...
df = pd.read_csv("features_scaled_pipeline_B_split.csv")  # output of pipeline_B_split.py
...
if "Split" not in df.columns:
    raise ValueError("Expected a 'Split' column from pipeline_B_split.py — got none.")
...
y = train_df["Class"].values  # 4-class labels
...
print(report)
output_lines = [
    "GridSearchCV - SVM Classification (4-class AMP) - Pipeline B",
    "Src: https://pmc.ncbi.nlm.nih.gov/articles/PMC12848716/",
    "This script runs before mRMR to get the k that feeds mRMR's max_features selected. Only applied on the train partition for pipeline B."
...
with open("classification_gridsearch_results_pipelineB.txt", "w") as f:
    f.write("\n".join(output_lines))
