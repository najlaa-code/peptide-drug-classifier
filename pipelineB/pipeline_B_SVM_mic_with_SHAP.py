...
Trains an SVM (RBF, class_weight="balanced") on the LASSO-selected MIC features
from pipeline A, using the same train/test split as the rest of pipeline A. Reports
classification metrics + 5-fold CV, and saves a confusion matrix plot.

Trains a 2nd SVM (same parameters, probability=True since KernelExplainer needs
predict_proba). Runs SHAP on it to see which descriptors are actually driving the
predictions, with special emphasis on the minority against the majority class.
...
INPUT_CSV = os.path.join(BASE_DIR, "lasso_pipeline_B_MIC_features.csv")
print("MIC results (Pipeline B):")
OUT_CM    = os.path.join(BASE_DIR, "svm_confusion_matrix_pipeline_B_MIC.png")
...
train_mask = df["Split"] == "train"  # using the same test/train split disseminated from pipeline_B_split.py
...
ax.set_title("SVM Confusion Matrix Pipeline B (MIC features)", fontsize=14)
plt.tight_layout()
...
imp.to_csv(os.path.join(BASE_DIR, "shap_pipeline_B_MIC_mean_abs.csv"))
...
for i, c in enumerate(classes):
    ...
    plt.title(f"SHAP mean |value| - {c} (MIC, Pipeline B)")
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f"shap_bar_pipeline_B_MIC_{safe(c)}.png"), dpi=150)
    plt.close()
