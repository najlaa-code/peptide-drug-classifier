...
INPUT_CSV = os.path.join(BASE_DIR, "lasso_pipeline_A_MIC_features.csv")
print("MIC results:")
OUT_CM    = os.path.join(BASE_DIR, "svm_confusion_matrix_pipeline_A_MIC.png")
...
ax.set_title("SVM Confusion Matrix Pipeline A (MIC features)", fontsize=14)
plt.tight_layout()
...
imp.to_csv(os.path.join(BASE_DIR, "shap_pipeline_A_mic_mean_abs.csv"))
...
for i, c in enumerate(classes):
    ...
    plt.title(f"SHAP mean |value| — {c} (MIC, Pipeline A)")
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f"shap_bar_pipeline_A_MIC_{safe(c)}.png"), dpi=150)
    plt.close()
