"""
SVM for Pipeline A (HC50 + MIC combined))
Trains an SVM (RBF, class_weight="balanced") on the union of the LASSO-selected HC50 and MIC features from 
Pipeline A, merging the two feature sets on SequenceIndex/Sequence and checking that any overlapping features 
match between the two before combining. Uses the same train/test split as the rest of Pipeline A. Reports 
...
Input:
- lasso_pipeline_A_HC50_features.csv and lasso_pipeline_A_MIC_features.csv (LASSO-selected feature matrices, 
...
- svm_confusion_matrix_pipeline_A_UNION.png
- shap_pipeline_A_union_with_shap_mean_abs.csv - mean |SHAP value| per feature, per class
- shap_bar_pipeline_A_UNION_<class>.png - one SHAP bar plot per class
"""
import pandas as pd
import numpy as np
...
HC50_CSV = os.path.join(BASE_DIR, "lasso_pipeline_A_HC50_features.csv")
MIC_CSV  = os.path.join(BASE_DIR, "lasso_pipeline_A_MIC_features.csv")
OUT_CM   = os.path.join(BASE_DIR, "svm_confusion_matrix_pipeline_A_UNION.png")
...
#based on pipeline_A_hyperparameter_search numbers, hardcoded
svm = SVC(
    kernel="rbf",
    C=1.0,
    gamma="scale",
...
ax.set_title("SVM Confusion Matrix Pipeline A (HC50+MIC features)", fontsize=14)
plt.tight_layout()
...
imp.to_csv(os.path.join(BASE_DIR, "shap_pipeline_A_union_with_shap_mean_abs.csv"))
...
for i, c in enumerate(classes):
    ...
    plt.title(f"SHAP mean |value| — {c} (Union HC50+MIC, Pipeline A)")
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f"shap_bar_pipeline_A_UNION_{safe(c)}.png"), dpi=150)
    plt.close()
