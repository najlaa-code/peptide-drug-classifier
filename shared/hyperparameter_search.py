...
sys.stdout= open("hyperparameters_search.txt", "w")
...
k_options= [int(total_features * p) for p in [0.05, 0.10, 0.20, 0.30]]
print(f"k options: {k_options}")
C_options = np.logspace(-3, 3, 7).tolist()
gamma_options = np.logspace(-3, 3, 7).tolist()
...
for k in k_options:
    for C in C_options:
        ...
        print(f"Tested: k={k}, C={C:.4f}, gamma={gamma:.4f}")

        if avg_cv_score > best_score:
            best_score = avg_cv_score
            best_params = {'k': k, 'C': C, 'gamma':gamma}

print(f"Best parameters: {best_params}")
print(f"Best CV balanced accuracy: {best_score:.4f}")
print(f"Best gamma: {best_params['gamma']:.5f}")
